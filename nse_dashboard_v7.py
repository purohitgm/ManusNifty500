...
def scanner_table(df_scan, key):
    """
    Selectable dataframe. Clicking a row:
      1. Updates st.session_state.chart_sym  (our tracking var)
      2. Updates st.session_state["chart_selectbox"]  (the widget's own key)
         → this is the critical step that actually moves the selectbox
    """
    if df_scan.empty:
        return
    evt = st.dataframe(
    ...
    if evt and hasattr(evt, "selection") and evt.selection and evt.selection.rows:
        ...
        if picked in stock_dfs and picked != st.session_state.chart_sym:
            st.session_state.chart_sym          = picked   # our var
            st.session_state["chart_selectbox"] = picked   # ← moves the selectbox widget
            st.rerun()
...
with cc1:
    # key="chart_selectbox" means st.session_state["chart_selectbox"] controls its value.
    # scanner_table() sets that key directly, so the widget moves automatically on rerun.
    chart_sym = st.selectbox(
        "Select Stock",
        options=list(stock_dfs.keys()),
        key="chart_selectbox",          # widget state is the source of truth
    )
    st.session_state.chart_sym = chart_sym   # keep tracking var in sync
...
for col in ["NR7", "NR4", "PocketPivot", "RS_NewHigh", "Above20DMA", "Above50DMA"]:
