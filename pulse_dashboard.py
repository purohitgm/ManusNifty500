...
Tabs: Overview · RRG · Screener · Breadth · FII/DII · Options · Watchlist · Alerts
"""
...
from data_fetcher import (
    ...
    fetch_sector_data, fetch_screener, fetch_fii_dii, fetch_options_data,
...
.timer-ring .fg{fill:none;stroke:#f0a500;stroke-width:3.5;stroke-linecap:round;stroke-dasharray:100;transition:stroke-dashoffset .9s linear}
.timer-label{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);font-size:.58rem;font-weight:700;color:#f0a500;font-family:'JetBrains Mono',monospace}
</style>
...
def countdown_html(elapsed):
    rem = max(0, AUTO_REFRESH - elapsed)
    pct = rem / AUTO_REFRESH
    off = 100 - int(pct * 100)
    mkt = is_market_open()
    dot = '<span class="live-dot"></span>' if mkt else '<span class="mkt-closed-dot"></span>'
    status = "LIVE" if mkt else "CLOSED"
    now = (datetime.utcnow() + timedelta(hours=5, minutes=30)).strftime("%H:%M:%S")
    return f"""<div style="display:flex;align-items:center;gap:12px">
      <div style="display:flex;align-items:center;gap:5px">{dot}
        <span style="font-size:.7rem;color:#8b949e;font-weight:700;letter-spacing:.06em">{status}</span>
      </div>
      <span style="font-size:.7rem;color:#8b949e;font-family:'JetBrains Mono',monospace">{now} IST</span>
      <div class="timer-ring">
        <svg viewBox="0 0 36 36" width="36" height="36">
          <circle class="bg" cx="18" cy="18" r="15.9"/>
          <circle class="fg" cx="18" cy="18" r="15.9" style="stroke-dashoffset:{off}"/>
        </svg>
        <div class="timer-label">{int(rem):02d}</div>
      </div>
    </div>"""
...
# ── Options OI Bar chart ──────────────────────────────────────────────────────
def render_options_oi(opts):
    if not opts.get("top_call_strikes"): return go.Figure()
    calls = opts["top_call_strikes"][:10]; puts = opts["top_put_strikes"][:10]
    fig = go.Figure()
    fig.add_trace(go.Bar(x=[c["strike"] for c in calls],y=[c["oi"] for c in calls],
        name="Call OI",marker_color="#f87171",opacity=.85))
    fig.add_trace(go.Bar(x=[p["strike"] for p in puts],y=[p["oi"] for p in puts],
        name="Put OI",marker_color="#4ade80",opacity=.85))
    if opts.get("spot"):
        fig.add_vline(x=opts["spot"],line=dict(color="#f0a500",dash="dash",width=2),
            annotation_text=f"Spot {opts['spot']:.0f}",annotation_font_color="#f0a500",annotation_font_size=10)
    if opts.get("max_pain"):
        fig.add_vline(x=opts["max_pain"],line=dict(color="#a78bfa",dash="dot",width=1.5),
            annotation_text=f"Max Pain {opts['max_pain']:.0f}",annotation_font_color="#a78bfa",
            annotation_font_size=10,annotation_position="bottom right")
    fig.update_layout(**PT,height=340,margin=dict(l=40,r=10,t=20,b=40),barmode="overlay",
        xaxis=dict(title="Strike Price",gridcolor="#1a2030",tickfont=dict(size=9)),
        yaxis=dict(title="Open Interest",gridcolor="#1a2030",tickfont=dict(size=9)),
        legend=dict(orientation="h",y=1.05,bgcolor="rgba(0,0,0,0)"))
    return fig
...
def main():
    ...
    with h3:
        ...
        with rc1: st.markdown(countdown_html(elapsed),unsafe_allow_html=True)
        with rc2:
        ...
        if results:
            ...
            for s in results:
                rows.append({"Grade":s.get("grade","—"),"Symbol":s["symbol"].replace(".NS",""),
                    ...
                    "Chg%":s["change_pct"],"RSI":s["rsi"] if not math.isnan(s.get("rsi",float("nan"))) else 0,
                    "Mom":s["momentum"],"VolR":s["vol_ratio"],"RS#":s.get("rs_rank",50),
    ...
    # ══════════ OPTIONS ════════════════════════════════════════════════════════
    with tabs[5]:
        idx_sel=st.selectbox("Index",["NIFTY","BANKNIFTY","FINNIFTY"],key="opts_idx")
        with st.spinner(f"Loading {idx_sel} options…"):
            opts=fetch_options_data(idx_sel)

        o1,o2,o3,o4,o5=st.columns(5)
        pcr=opts.get("pcr",0); sig=opts.get("pcr_signal","N/A")
        pcr_cls="pcr-bull" if sig=="Bullish" else "pcr-bear" if sig=="Bearish" else "pcr-neu"
        o1.markdown(f'<div style="background:#0d1117;border:1px solid #21262d;border-radius:8px;padding:11px">'
            f'<div style="font-size:.68rem;color:#8b949e;font-weight:700;letter-spacing:.07em">PUT/CALL RATIO</div>'
            f'<div class="{pcr_cls}">{pcr:.3f}</div>'
            f'<div style="font-size:.73rem;color:#8b949e">{sig}</div></div>',unsafe_allow_html=True)
        o2.metric("Spot",f"{opts.get('spot',0):,.0f}")
        o3.metric("Max Pain",f"{opts.get('max_pain',0):,.0f}")
        o4.metric("ATM IV",f"{opts.get('atm_iv',0):.1f}%")
        o5.metric("Call OI",f"{opts.get('total_call_oi',0)/100000:.1f}L")

        if opts.get("top_call_strikes"):
            st.markdown('<div class="section-hdr" style="margin-top:10px">Open Interest Distribution — Key Support/Resistance</div>',unsafe_allow_html=True)
            st.plotly_chart(render_options_oi(opts),use_container_width=True,config={"displayModeBar":False})

            # Top resistance / support
            oc1,oc2=st.columns(2)
            with oc1:
                st.markdown('<div class="section-hdr">Top Call OI (Resistance)</div>',unsafe_allow_html=True)
                df=pd.DataFrame(opts["top_call_strikes"])
                df.columns=["Strike","Call OI"]
                st.dataframe(df,use_container_width=True,hide_index=True,
                    column_config={"Strike":st.column_config.NumberColumn(format="%.0f"),
                        "Call OI":st.column_config.NumberColumn(format="%,d")})
            with oc2:
                st.markdown('<div class="section-hdr">Top Put OI (Support)</div>',unsafe_allow_html=True)
                df=pd.DataFrame(opts["top_put_strikes"])
                df.columns=["Strike","Put OI"]
                st.dataframe(df,use_container_width=True,hide_index=True,
                    column_config={"Strike":st.column_config.NumberColumn(format="%.0f"),
