...
# RRG COMPUTATION
# ─────────────────────────────────────────────────────────────────
def _rrg_row(name, price_series, bench_s, tail=10):
    """Compute one RRG row from a price series vs benchmark."""
    al = pd.concat([price_series, bench_s], axis=1).dropna()
    if len(al) < 60:
        return None
    al.columns = ["px", "bench"]
    pr       = (al["px"] / al["bench"]) / (al["px"].iloc[0] / al["bench"].iloc[0]) * 100
    ...
    r, m = float(rs_ratio.iloc[-1]), float(rs_mom.iloc[-1])
    quad = ("Leading"   if r >= 100 and m >= 100 else
            "Weakening" if r >= 100 and m <  100 else
            "Improving" if r <  100 and m >= 100 else "Lagging")
    return {
        "Name": name, "RS_Ratio": round(r, 4), "RS_Momentum": round(m, 4),
        "Quadrant": quad,
        "tail_ratio": [round(x, 4) for x in rs_ratio.iloc[-tail:].tolist()],
        "tail_mom":   [round(x, 4) for x in rs_mom.iloc[-tail:].tolist()],
    }

def compute_rrg(price_dict, bench_s, tail=10):
    """RRG for group indices (sector / industry)."""
    rows = [_rrg_row(name, ps, bench_s, tail)
            for name, ps in price_dict.items()]
    return pd.DataFrame([r for r in rows if r is not None])

def compute_rrg_stocks(sym_list, bench_s, tail=10):
    """RRG for individual stocks — used when user drills into an industry."""
    rows = []
    for sym in sym_list:
        try:
            ps = data[sym]["Close"].dropna()
        except Exception:
            continue
        row = _rrg_row(sym.replace(".NS", ""), ps, bench_s, tail)
        if row:
            rows.append(row)
...
def rrg_chart(rrg_df, title=""):
    """
    Fixed RRG scatter chart:
     • Quadrant colour fills via markArea on a dummy series
     • Centrepiece cross at (100, 100) via markLine
     • Per-name tail lines (dashed) + current dot with label
     • labelLayout moveOverlap prevents label collisions
     • Quadrant corner labels anchored with graphic text elements
    """
    QFILL = {
        "Leading":   "rgba(38,166,154,0.13)",
        "Weakening": "rgba(255,193,7,0.13)",
        "Lagging":   "rgba(239,83,80,0.13)",
        "Improving": "rgba(100,181,246,0.13)",
    }
    QDOT = {
        "Leading":   "#26A69A",
        "Weakening": "#FFC107",
    ...
    all_r = [v for _, row in rrg_df.iterrows() for v in row["tail_ratio"]]
    all_m = [v for _, row in rrg_df.iterrows() for v in row["tail_mom"]]
    pad   = 0.6
    xmin  = min(min(all_r) - pad, 99.2)
    xmax  = max(max(all_r) + pad, 100.8)
    ymin  = min(min(all_m) - pad, 99.2)
    ymax  = max(max(all_m) + pad, 100.8)

    series = []

    # ── 1. Quadrant fill via markArea ─────────────────────────────
    series.append({
    ...
    # ── 2. Centre cross at (100, 100) via markLine ────────────────
    series.append({
        "type": "scatter", "data": [], "silent": True, "z": 1,
        "markLine": {
            "silent":    True,
            "symbol":    ["none", "none"],
            "lineStyle": {"color": "rgba(255,255,255,0.25)", "width": 1.5, "type": "solid"},
            "data": [
                [{"coord": [100, ymin]}, {"coord": [100, ymax]}],  # vertical
                [{"coord": [xmin, 100]}, {"coord": [xmax, 100]}],  # horizontal
            ],
        },
    })

    # ── 3. Tail lines + current dots per name ────────────────────
    for _, row in rrg_df.iterrows():
        c   = QDOT[row["Quadrant"]]
        pts = [[r, m] for r, m in zip(row["tail_ratio"], row["tail_mom"])]

        # tail line (dashed, faded)
        series.append({
            "type":       "line",
            "data":       pts,
            "showSymbol": False,
            "lineStyle":  {"color": c, "width": 1.4, "type": "dashed", "opacity": 0.55},
            "z":          2,
            "silent":     True,
        })
        ...
        series.append({
            "type":       "scatter",
            "name":       row["Name"],
            "data":       [[row["RS_Ratio"], row["RS_Momentum"]]],
            "symbolSize": 16,
            "itemStyle":  {"color": c, "borderColor": "#fff", "borderWidth": 1.5},
            "label": {
                "show":      True,
                "formatter": "{a}",        # uses series name
                "position":  "top",
                "color":     "#fff",
                "fontSize":  10,
                "fontWeight": "bold",
            ...
            "labelLayout": {"moveOverlap": "shiftY"},   # ← prevents overlap
            "tooltip": {
                "formatter": (
                    f"<b>{row['Name']}</b><br/>"
                    f"Quadrant : <b style='color:{c}'>{row['Quadrant']}</b><br/>"
    ...
    opt = {
        ...
        # Quadrant corner text via graphic layer
        "graphic": [
            {"type": "text", "z": 5,
             "style": {"text": "Leading",   "fill": QLABEL_COLOR["Leading"],
                       "font": "bold 13px sans-serif"},
             "right": "10%", "top": "13%"},
            {"type": "text", "z": 5,
             "style": {"text": "Weakening", "fill": QLABEL_COLOR["Weakening"],
                       "font": "bold 13px sans-serif"},
             "right": "10%", "bottom": "13%"},
            {"type": "text", "z": 5,
             "style": {"text": "Lagging",   "fill": QLABEL_COLOR["Lagging"],
                       "font": "bold 13px sans-serif"},
             "left": "10%", "bottom": "13%"},
            {"type": "text", "z": 5,
             "style": {"text": "Improving", "fill": QLABEL_COLOR["Improving"],
                       "font": "bold 13px sans-serif"},
             "left": "10%", "top": "13%"},
        ],
        "series": series,
    ...
    if title:
        opt["title"] = {"text": title, "textStyle": {"color": TEXT, "fontSize": 12}, "left": "center", "top": 4}
    return opt
...
# 2. RRG  ── 3-level drill-down: All Sectors → Sector Industries → Industry Stocks
st.subheader("🔄 Relative Rotation Graph (RRG)")

qi = {"Leading": "🟢", "Weakening": "🟡", "Lagging": "🔴", "Improving": "🔵"}

rrg_tab1, rrg_tab2, rrg_tab3 = st.tabs(["📊 All Sectors", "🏭 Sector → Industries", "📈 Industry → Stocks"])

# ── Tab 1 : All Sectors ───────────────────────────────────────────
with rrg_tab1:
    if not rrg_sec.empty:
        st_echarts(rrg_chart(rrg_sec, "All Sectors — RS-Ratio vs RS-Momentum"), height=vh(74))
        d_sec = rrg_sec[["Name", "Quadrant", "RS_Ratio", "RS_Momentum"]].copy()
        d_sec["Quadrant"] = d_sec["Quadrant"].map(lambda q: f"{qi.get(q,'')} {q}")
        st.dataframe(d_sec.sort_values("Quadrant"), use_container_width=True, hide_index=True)
    else:
        st.info("RRG unavailable — benchmark data missing.")

# ── Tab 2 : Industries within a chosen Sector ─────────────────────
with rrg_tab2:
    if benchmark_close is not None:
        all_sectors = sorted(sec_map.keys())
        sel_sector  = st.selectbox(
            "Select Sector to explore its Industries",
            options=all_sectors,
            key="rrg_sector_sel",
        )
        # Build industry-level price dict for only the chosen sector's industries
        sector_inds = {}
        for sym, (sec, ind) in stocks.items():
            if sec == sel_sector:
                sector_inds.setdefault(ind, []).append(sym)

        sector_ind_price = {k: _make_index(v) for k, v in sector_inds.items()}
        sector_ind_price = {k: v for k, v in sector_ind_price.items() if v is not None}
        rrg_sel_ind      = compute_rrg(sector_ind_price, benchmark_close)

        if not rrg_sel_ind.empty:
            st_echarts(
                rrg_chart(rrg_sel_ind, f"{sel_sector} — Industries RRG"),
                height=vh(74),
            )
            d_ind = rrg_sel_ind[["Name", "Quadrant", "RS_Ratio", "RS_Momentum"]].copy()
            d_ind["Quadrant"] = d_ind["Quadrant"].map(lambda q: f"{qi.get(q,'')} {q}")
            st.dataframe(d_ind.sort_values("Quadrant"), use_container_width=True, hide_index=True)
        else:
            st.warning(f"Not enough data to build RRG for {sel_sector} industries.")
    else:
        st.info("RRG unavailable — benchmark data missing.")

# ── Tab 3 : Individual Stocks within a chosen Industry ────────────
with rrg_tab3:
    if benchmark_close is not None:
        # Sector picker → then industry picker
        t3_col1, t3_col2 = st.columns(2)
        with t3_col1:
            all_sectors_t3 = sorted(sec_map.keys())
            sel_sec_t3     = st.selectbox(
                "1️⃣  Select Sector",
                options=all_sectors_t3,
                key="rrg_sec_t3",
            )
        with t3_col2:
            # Industries that belong to the selected sector
            inds_in_sec = sorted({ind for sym, (sec, ind) in stocks.items() if sec == sel_sec_t3})
            sel_ind_t3  = st.selectbox(
                "2️⃣  Select Industry",
                options=inds_in_sec,
                key="rrg_ind_t3",
            )

        # Stocks in the selected industry
        stocks_in_ind = [sym for sym, (sec, ind) in stocks.items()
                         if sec == sel_sec_t3 and ind == sel_ind_t3]

        rrg_stocks_df = compute_rrg_stocks(stocks_in_ind, benchmark_close)

        if not rrg_stocks_df.empty:
            st_echarts(
                rrg_chart(rrg_stocks_df, f"{sel_ind_t3}  ({sel_sec_t3}) — Individual Stocks RRG"),
                height=vh(74),
            )
            d_stk = rrg_stocks_df[["Name", "Quadrant", "RS_Ratio", "RS_Momentum"]].copy()
            d_stk["Quadrant"] = d_stk["Quadrant"].map(lambda q: f"{qi.get(q,'')} {q}")
            st.dataframe(d_stk.sort_values("Quadrant"), use_container_width=True, hide_index=True)
        else:
            st.warning(f"Not enough data for stocks in {sel_ind_t3}.")
    else:
        st.info("RRG unavailable — benchmark data missing.")
